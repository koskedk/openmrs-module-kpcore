...
package org.openmrs.module.kpcore.form;
...
public class FormUtilsTest extends BaseModuleContextSensitiveTest {
	...
	/**
	 * @see org.openmrs.module.kpcore.form.FormUtils#getFormXmlResource(org.openmrs.Form)
	 */
	@Test
	public void getFormXmlPath_shouldReturnXmlPathIfExists() {
		...
		resource.setValue("kpcore:test1.xml");
		Context.getFormService().saveFormResource(resource);

		Assert.assertEquals(new UiResource("kpcore", "test1.xml"), FormUtils.getFormXmlResource(form));
	}

	@Test
	public void setFormXmlPath_shouldSetPathAsFormResource() {
		Form form = Context.getFormService().getForm(1);
		FormUtils.setFormXmlResource(form, new UiResource("kpcore", "test2.xml"));

		FormResource resource = Context.getFormService().getFormResource(form, FormUtils.RESOURCE_HFE_XML_PATH);
		Assert.assertEquals("kpcore:test2.xml", resource.getValue());
	}

	@Test
	public void getHtmlForm_shouldCreateDynamicHtmlFormFormXmlPathResource() throws Exception {
		Form form = Context.getFormService().getForm(1);
		FormUtils.setFormXmlResource(form, new UiResource("kpcore", "test3.xml"));

		// Mock the resource factory so it will provide this xml content at kpcore:test3.xml
		String xmlContent = "<htmlform>Test</htmlform>";
		ResourceFactory resourceFactory = mock(ResourceFactory.class);
		when(resourceFactory.getResourceAsString("kpcore", "htmlforms/test3.xml")).thenReturn(xmlContent);
